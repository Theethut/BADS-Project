...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device, g_source, d_source):
    ...
    imsave_path = os.path.join('/content/drive/MyDrive/FEWSHOTGAN/data/samples', args.exp) #image path #os.path.join('/content/fewshotgan/data/samples', args.exp)
    model_path = os.path.join('/content/drive/MyDrive/FEWSHOTGAN/data/checkpoints', args.exp) #model path #os.path.join('/content/fewshotgan/data/checkpoints', args.exp)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iter", type=int, default=50002) #parser.add_argument("--iter", type=int, default=5002)
    parser.add_argument("--save_freq", type=int, default=2500) #2500 #save model #1000
    parser.add_argument("--img_freq", type=int, default=2500) #2500 #gen image #500
    ...
    parser.add_argument("--n_train", type=int, default=111) #parser.add_argument("--n_train", type=int, default=10)

    args = parser.parse_args()
    ###
    print(f'save model freq: {args.save_freq}')
    print(f'save image freq: {args.img_freq}')
    print(f'n image trained from: {args.n_train}')
    ...
    if args.ckpt is not None:
        ...
        #assert args.source_key in args.ckpt #comment
        ...
        generator.load_state_dict(ckpt["g"], strict=False) #comment
        g_source.load_state_dict(ckpt_source["g"], strict=False) #comment
        g_ema.load_state_dict(ckpt["g_ema"], strict=False) #comment

        #d_source = nn.parallel.DataParallel(d_source)
        #discriminator = nn.parallel.DataParallel(discriminator)
        discriminator.load_state_dict(ckpt["d"]) #comment
        d_source.load_state_dict(ckpt_source["d"]) #comment
